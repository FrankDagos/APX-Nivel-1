...
for (let i = 0; i < temperaturaPorHora.length; i++) {
  const { fecha, hora, temperatura } = temperaturaPorHora[i];

  // Agregamos la temperatura al array correspondiente según la fecha
  temperaturasPorDia[fecha].push({ hora, temperatura });
}

// Ahora puedes ver las temperaturas agrupadas por día
console.log(temperaturasPorDia);

for (const dia in temperaturasPorDia) {
  let suma = 0;
  let contador = 0;
  for (let i = 0; i < temperaturasPorDia[dia].length; i++) {
    let temperaturaCadaHora = temperaturasPorDia[dia][i].temperatura;
    suma += temperaturaCadaHora;
    contador++;
  }

  let promedio4 = suma / contador;
  console.log(`El promedio del dia ${dia} por hora es de ${promedio4}`);
}
